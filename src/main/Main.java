...
public class Main {
	...
	public static void main(String[] args) {
		...
		Equipo realBetis = new Equipo("Real Betis", "Sevilla", laLiga, 1907, 0, "Benito Villamarín", "realbetis"); //Error en logo
		...
		Equipo rayo = new Equipo("Rayo Vallecano", "Madrid", laLiga, 1924, 0, "Campo de Vallecas", "rayo"); //Error en logo
		...
		Equipo laspalmas = new Equipo("UD Las Palmas", "Las Palmas", laLiga, 1949, 0, "Gran Canaria", "laspalmas"); //Error en logo
		Equipo alaves = new Equipo("Deportivo Alavés", "Vitoria", laLiga, 1921, 0, "Mendizorroza", "alaves");
		Equipo valladolid = new Equipo("Real Valladolid", "Valladolid", laLiga, 1928, 0, "José Zorrilla", "valladolid"); //Error en logo
		Equipo leganes = new Equipo("CD Leganés", "Leganés", laLiga, 1928, 0, "Butarque", "leganes"); //Error en logo
		...
		laLigaEquipos.add(laspalmas);
		laLigaEquipos.add(alaves);
		laLigaEquipos.add(valladolid);
		laLigaEquipos.add(leganes);
		...
		Equipo leicester = new Equipo("Leicester City", "Leicester", premier, 1884, 1, "King Power Stadium", "leicester"); //Error en logo
		...
		Equipo nottingham = new Equipo("Nottingham Forest", "Nottingham", premier, 1865, 2, "City Ground", "nottingham"); //Error en logo
		Equipo southampton = new Equipo("Southampton FC", "Southampton", premier, 1885, 1, "St Mary's Stadium", "southampton"); //Error en logo
		Equipo ipswich = new Equipo("Ipswich Town", "Ipswich", premier, 1878, 1, "Portman Road", "ipswich"); //Error en logo
		...
		premierLeagueEquipos.add(leicester);
		...
		premierLeagueEquipos.add(southampton);
		premierLeagueEquipos.add(ipswich);

		premier.setEquipos(premierLeagueEquipos);

		// ====== Bundesliga (20 equipos) ======
		...
		Equipo eintracht = new Equipo("Eintracht Frankfurt", "Frankfurt", bundesliga, 1899, 1, "Deutsche Bank Park", "eintracht"); //Error en logo
		...
		Equipo mainz = new Equipo("FSV Mainz 05", "Maguncia", bundesliga, 1905, 0, "Mewa Arena", "mainz"); //Error en logo
		Equipo augsburg = new Equipo("FC Augsburg", "Augsburgo", bundesliga, 1907, 0, "WWK Arena", "augsburg"); //Error en logo
		Equipo werderbremen = new Equipo("Werder Bremen", "Bremen", bundesliga, 1899, 4, "Weserstadion", "werderbremen");
		Equipo bochum = new Equipo("VfL Bochum", "Bochum", bundesliga, 1848, 0, "Vonovia Ruhrstadion", "bochum"); //Error en logo
		Equipo stpauli = new Equipo("FC St. Pauli", "Hamburgo", bundesliga, 1910, 0, "Millerntor-Stadion", "stpauli"); //Error en logo
		Equipo heidenheim = new Equipo("1. FC Heidenheim", "Heidenheim", bundesliga, 1846, 0, "Voith-Arena", "heidenheim");
		Equipo holstein = new Equipo("Holstein Kiel", "Kiel", bundesliga, 1900, 0, "Holstein-Stadion", "holstein"); //Error en logo
		Equipo darmstadt = new Equipo("SV Darmstadt 98", "Darmstadt", bundesliga, 1898, 0, "Merck-Stadion", "darmstadt"); //Error en logo
		Equipo koln = new Equipo("1. FC Köln", "Colonia", bundesliga, 1948, 2, "RheinEnergieStadion", "koln");
		...
		bundesligaEquipos.add(bochum);
		bundesligaEquipos.add(stpauli);
		bundesligaEquipos.add(heidenheim);
		bundesligaEquipos.add(holstein);
		bundesligaEquipos.add(darmstadt);
